...
<head>
    ...
    <title>Job Resources | LSPU - EIS</title>
...
<body :class="[darkMode ? 'dark' : '', 'font-sans bg-gray-50 dark:bg-gray-800 min-h-screen']" id="app" v-cloak>
    ...
    <!-- Header -->
        ...
        <div class="overflow-y-auto pt-4 pb-20 h-[calc(100%-64px)] scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 dark:scrollbar-thumb-slate-600 dark:scrollbar-track-slate-800/50">
            ...
            <!-- Job Resources -->
            <a href="employer_resources" class="flex items-center px-6 py-3 mx-2 rounded-lg bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 hover:bg-blue-500/20 dark:hover:bg-blue-500/30 transition-colors duration-200 border-l-4 border-blue-500 dark:border-blue-400" @click="handleNavClick">
        ...
        <div class="container-fluid max-w-7xl mx-auto">
            <div class="bg-white dark:bg-gray-700 rounded-xl shadow-xl p-6">
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                    <h2 class="text-2xl font-bold mb-2 md:mb-0 text-gray-800 dark:text-gray-100">Job Resources</h2>
                ...
                <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                    <div class="flex items-center gap-2 w-full md:w-auto mb-2 md:mb-0">
                        <div class="relative w-full md:w-80">
                            ...
                            <input type="text" class="form-input w-full pl-10 px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Search resources..." v-model="searchQuery" @input="filterResources">
                        </div>
                    </div>
                    <div class="flex flex-col sm:flex-row flex-wrap gap-2 w-full md:w-auto">
                        <select class="form-select px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 w-full sm:w-auto lg:w-[200px]" v-model="filters.job">
                            <option value="">All Jobs</option>
                            <option v-for="job in jobOptions" :value="job.job_id">{{ job.title }}</option>
                        </select>
                        <select class="form-select px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 w-full sm:w-auto lg:w-[180px]" v-model="filters.status">
                        ...
                        <!-- Third Select Filter - Department -->
                        <select class="form-select px-3 py-2 rounded border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 text-gray-800 dark:text-gray-100 w-full sm:w-auto lg:w-[200px]" v-model="filters.department">
                    ...
                    <table class="min-w-full text-sm text-center">
                        <thead>
                            <tr class="bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-200">
                                ...
                                <th class="px-4 py-2 text-center">Budget</th>
                                <th class="px-4 py-2 text-center">Status</th>
                                <th class="px-4 py-2 text-center">Timeline</th>
                                <th class="px-4 py-2 text-center">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="resource in paginatedResources" :key="resource.id" class="border-b border-gray-200 dark:border-gray-600">
                                <td class="px-4 py-2 font-semibold text-gray-800 dark:text-gray-200 text-center">{{ resource.department }}</td>
                                <td class="px-4 py-2 text-gray-800 dark:text-gray-200 text-center">{{ getJobTitle(resource.job_id) }}</td>
                                <td class="px-4 py-2 text-gray-800 dark:text-gray-200 text-center">₱{{ formatCurrency(resource.financial_budget) }}</td>
                                <td class="px-4 py-2 text-center">
                                    <span :class="['inline-block px-2 py-1 rounded text-xs font-semibold', 
                                        resource.status === 'Completed' ? 'bg-green-100 text-green-700 dark:bg-green-800 dark:text-green-200' : 
                                        resource.status === 'In Progress' ? 'bg-blue-100 text-blue-700 dark:bg-blue-800 dark:text-blue-200' : 
                                        resource.status === 'Approved' ? 'bg-purple-100 text-purple-700 dark:bg-purple-800 dark:text-purple-200' : 
                                        'bg-yellow-100 text-yellow-700 dark:bg-yellow-800 dark:text-yellow-200']">
                                        {{ resource.status }}
                                    </span>
                                </td>
                                <td class="px-4 py-2 text-gray-800 dark:text-gray-200 text-center">{{ resource.timeline || 'N/A' }}</td>
                                <td class="px-4 py-2 text-center">
                                    <div class="relative inline-block text-left">
                                        <button @click="toggleActionDropdown(resource.id)" class="p-2 rounded hover:bg-gray-200 dark:hover:bg-gray-600 focus:outline-none text-gray-500 dark:text-gray-200">
                                            <i class="fas fa-ellipsis-h"></i>
                                        </button>
                                        <div v-if="actionDropdown === resource.id" class="origin-top-right absolute right-0 mt-2 w-32 rounded-md shadow-lg bg-white dark:bg-gray-700 ring-1 ring-black ring-opacity-5 z-10">
                                            <div class="py-1">
                                                <a href="#" class="block px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600" @click.prevent="viewResource(resource)"><i class="fas fa-eye mr-2"></i>View</a>
                                                <a href="#" class="block px-4 py-2 text-sm text-yellow-600 hover:bg-yellow-100 dark:hover:bg-yellow-800" @click.prevent="openEditModal(resource)"><i class="fas fa-edit mr-2"></i>Edit</a>
                                                <a href="#" class="block px-4 py-2 text-sm text-red-600 hover:bg-red-100 dark:hover:bg-red-800" @click.prevent="confirmDelete(resource)"><i class="fas fa-trash mr-2"></i>Delete</a>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                            ...
                            <tr v-if="filteredResources.length === 0">
                                <td colspan="6" class="py-12 text-center text-gray-500 dark:text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <span class="text-lg text-gray-400">No resources found</span>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
            ...
            <!-- Replace the entire Resource Form Modal section with this corrected version -->
            <div v-if="showResourceModal && (modalMode === 'add' || modalMode === 'edit')" class="fixed inset-0 z-[210] flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-4xl mx-4 p-6 relative max-h-[90vh] overflow-y-auto">
                    <button class="absolute top-4 right-4 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" @click="closeResourceModal">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                    
                    <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-gray-100">
                        {{ modalMode === 'edit' ? 'Edit' : 'Add' }} Job Resource
                    </h2>
                    
                    <form @submit.prevent="modalMode === 'edit' ? updateResource() : addResource()" class="space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Department:</label>
                                <input type="text" v-model="resourceForm.department" required 
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                            </div>
                            
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Financial Budget:</label>
                                <input type="number" v-model="resourceForm.financial_budget" step="0.01"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>

                        <div class="form-group">
                            <label for="resourceJob" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Select Job*:</label>
                            <select id="resourceJob" v-model="resourceForm.job_id" required
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                        ...
                        <div class="form-group">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Technology Needed:</label>
                            <textarea v-model="resourceForm.technology_needed" rows="3"
                        ...
                        <div class="form-group">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Training Required:</label>
                            <textarea v-model="resourceForm.training_required" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Physical Requirements/Tools:</label>
                            <textarea v-model="resourceForm.physical_objects" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"></textarea>
                        </div>
                        
                        <div class="form-group">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Staffing Requirements:</label>
                            <textarea v-model="resourceForm.staffing_requirements" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"></textarea>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="form-group">
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Timeline:</label>
                                <input type="text" v-model="resourceForm.timeline"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea v-model="resourceForm.notes" rows="3"
                                    class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"></textarea>
                        </div>
                        
                        <div class="flex justify-end gap-3 pt-4">
                            <button type="button" @click="closeResourceModal" 
                            ...
                            <button type="submit" 
                                ...
                                {{ modalMode === 'edit' ? 'Update' : 'Add' }} Resource
                            </button>
                        </div>
                    </form>
                </div>
        ...
        <!-- Delete Confirmation Modal -->
            <div v-if="showDeleteModal" class="fixed inset-0 z-[200] flex items-center justify-center bg-black bg-opacity-50">
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-lg w-full max-w-md mx-2 p-6 relative">
                    ...
                    <p class="mb-6 text-gray-700 dark:text-gray-200">Are you sure you want to delete this resource?</p>
                    <div class="flex justify-end gap-2">
                        ...
                        <button class="px-4 py-2 rounded bg-red-600 text-white hover:bg-red-700 transition" @click="deleteResource">Delete</button>
                    </div>
                </div>
            </div>
            <!-- View Resource Modal - Enhanced Version -->
            <!-- View Resource Modal - Enhanced Version -->
            <div v-if="showResourceModal && modalMode === 'view'" class="fixed inset-0 z-[210] flex items-center justify-center bg-black bg-opacity-50 pointer-events-auto" role="dialog" aria-modal="true" data-modal="view-resource-modal">
                <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-4xl mx-2 p-0 relative max-h-[95vh] overflow-y-auto pointer-events-auto">
                    <button class="absolute top-3 right-3 flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full shadow hover:bg-gray-200 dark:hover:bg-gray-600 transition text-base font-semibold z-20" @click="closeResourceModal" id="close-view-resource-modal-btn">
                    ...
                    <div class="rounded-t-2xl bg-gradient-to-r from-blue-600 via-blue-500 to-blue-400 dark:from-blue-900 dark:via-blue-800 dark:to-blue-700 px-0 pt-6 pb-8 flex flex-col items-center relative">
                        <div class="absolute top-4 left-4 bg-white dark:bg-gray-700 rounded-full p-2 shadow-lg">
                            <i class="fas fa-file-alt text-blue-600 dark:text-blue-300 text-2xl"></i>
                        </div>
                        ...
                        <h3 class="text-3xl font-extrabold text-white drop-shadow-lg mb-1 text-center">{{ selectedResource.department }}</h3>
                        <span :class="['inline-block mt-1 px-3 py-1 rounded-full text-xs font-semibold shadow', 
                            selectedResource.status === 'Completed' ? 'bg-green-100 text-green-700 dark:bg-green-800 dark:text-green-200' : 
                            selectedResource.status === 'In Progress' ? 'bg-blue-100 text-blue-700 dark:bg-blue-800 dark:text-blue-200' : 
                            selectedResource.status === 'Approved' ? 'bg-purple-100 text-purple-700 dark:bg-purple-800 dark:text-purple-200' : 
                            'bg-yellow-100 text-yellow-700 dark:bg-yellow-800 dark:text-yellow-200']">
                    ...
                    <div class="px-6 py-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 shadow-sm mb-6">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="font-semibold text-gray-700 dark:text-gray-200">Job:</span> 
                                <span class="ml-1 text-gray-700 dark:text-gray-200">{{ getJobTitle(selectedResource.job_id) }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <i class="fas fa-building text-blue-500 dark:text-blue-300 w-5"></i>
                                <span class="font-semibold text-gray-700 dark:text-gray-200">Department:</span> 
                                <span class="ml-1 text-gray-700 dark:text-gray-200">{{ selectedResource.department }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <i class="fas fa-money-bill-wave text-blue-500 dark:text-blue-300 w-5"></i>
                                <span class="font-semibold text-gray-700 dark:text-gray-200">Budget:</span> 
                                <span class="ml-1 text-gray-700 dark:text-gray-200">₱{{ formatCurrency(selectedResource.financial_budget) }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <i class="fas fa-calendar-alt text-blue-500 dark:text-blue-300 w-5"></i>
                                <span class="font-semibold text-gray-700 dark:text-gray-200">Timeline:</span> 
                                <span class="ml-1 text-gray-700 dark:text-gray-200">{{ selectedResource.timeline || 'Not specified' }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <i class="fas fa-calendar-plus text-blue-500 dark:text-blue-300 w-5"></i>
                                <span class="font-semibold text-gray-700 dark:text-gray-200">Created Date:</span> 
                                <span class="ml-1 text-gray-700 dark:text-gray-200">{{ formatDate(selectedResource.created_at) }}</span>
                            </div>
                            <div class="flex items-center gap-3">
                                <i class="fas fa-tasks text-blue-500 dark:text-blue-300 w-5"></i>
                                <span class="font-semibold text-gray-700 dark:text-gray-200">Status:</span> 
                                <span class="ml-1 text-gray-700 dark:text-gray-200">{{ selectedResource.status }}</span>
                            </div>
                        </div>

                    
                        <h4 class="text-lg font-bold mb-3 text-gray-800 dark:text-gray-100 flex items-center gap-2">
                            <i class="fas fa-microchip text-blue-500 dark:text-blue-300"></i> 
                            <span>Technology Needed</span>
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 shadow-sm mb-6">
                            <p class="text-gray-700 dark:text-gray-200 whitespace-pre-line">{{ selectedResource.technology_needed || 'No technology requirements specified' }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 shadow-sm mb-6">
                            <p class="text-gray-700 dark:text-gray-200 whitespace-pre-line">{{ selectedResource.training_required || 'No training requirements specified' }}</p>
                        </div>

                        <h4 class="text-lg font-bold mb-3 text-gray-800 dark:text-gray-100 flex items-center gap-2">
                            <i class="fas fa-users text-blue-500 dark:text-blue-300"></i> 
                            <span>Physical Requirements/Tools</span>
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 shadow-sm mb-6">
                            <p class="text-gray-700 dark:text-gray-200 whitespace-pre-line">{{ selectedResource.physical_object || 'No staffing requirements specified' }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 shadow-sm mb-6">
                            <p class="text-gray-700 dark:text-gray-200 whitespace-pre-line">{{ selectedResource.staffing_requirements || 'No staffing requirements specified' }}</p>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 dark:bg-gray-900 rounded-xl p-4 shadow-sm mb-6">
                            <p class="text-gray-700 dark:text-gray-200 whitespace-pre-line">{{ selectedResource.notes || 'No additional notes' }}</p>
                        </div>
                        
                        <!-- Action Buttons -->
                        <div class="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                            <button @click="openEditModal(selectedResource)" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors flex items-center gap-2">
                                <i class="fas fa-edit"></i> Edit Resource
                            </button>
                            <button @click="closeResourceModal" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                                Close
                            </button>
                        </div>
                    </div>
    ...
    <script src="js/employer_resources.js"></script>
</body>
</html>