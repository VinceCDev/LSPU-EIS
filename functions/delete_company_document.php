...
$stmt = $db->prepare('SELECT employer_id, document_file FROM employer WHERE user_id = ? LIMIT 1');
$stmt->bind_param('i', $user_id);
$stmt->execute();
$stmt->bind_result($employer_id, $document_file);
$stmt->fetch();
$stmt->close();
if (!$employer_id) {
...
if ($document_file) {
    $file_path = '../../uploads/documents/' . $document_file;
