...
$stmt = $db->prepare('SELECT employer_id, company_logo FROM employer WHERE user_id = ? LIMIT 1');
$stmt->bind_param('i', $user_id);
$stmt->execute();
$stmt->bind_result($employer_id, $company_logo);
$stmt->fetch();
$stmt->close();
if (!$employer_id) {
...
if ($company_logo) {
    $file_path = '../../uploads/logos/' . $company_logo;
