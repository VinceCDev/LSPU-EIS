...
<head>
    ...
    <title>Administrator Messages | LSPU - EIS</title>
...
<body :class="[darkMode ? 'dark' : '', 'font-sans bg-gray-300 dark:bg-gray-800 min-h-screen overflow-hidden']" id="app" v-cloak>
    ...
    <div v-if="sidebarActive" class="fixed top-0 left-0 bottom-0 w-[280px] bg-slate-50 dark:bg-slate-800 text-slate-800 dark:text-slate-200 shadow-xl z-50 transition-all duration-300 ease-in-out transform md:translate-x-0" :class="{'-translate-x-full': !sidebarActive && isMobile}">
            ...
            <div class="overflow-y-auto pt-4 pb-20 h-[calc(100%-64px)] scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-slate-100 dark:scrollbar-thumb-slate-600 dark:scrollbar-track-slate-800/50">
                ...
                <!-- Messages -->
                <a href="admin_message" class="flex items-center px-6 py-3 mx-2 rounded-lg bg-blue-500/10 dark:bg-blue-500/20 text-blue-600 dark:text-blue-400 hover:bg-blue-500/20 dark:hover:bg-blue-500/30 transition-colors duration-200 border-l-4 border-blue-500 dark:border-blue-400" @click="handleNavClick">
            ...
            <div class="w-full md:w-3/4 bg-white dark:bg-gray-700 rounded-lg shadow p-4">
                ...
                <div v-else>
                    <div class="border-b-2 border-blue-700 dark:border-blue-300 pb-2 mb-4 flex items-center justify-between">
                        ...
                        <!-- Search bar styled like admin_job.php -->
                    ...
                    <div class="overflow-x-auto">
                        ...
                        <table v-if="activeFolder === 'inbox'" class="min-w-full text-base select-none">
                            ...
                            <tbody>
                                <tr 
                                    ...
                                    :key="msg.id" 
                                    class="hover:bg-blue-50 dark:hover:bg-blue-800 cursor-pointer transition" 
                        ...
                        <table v-if="activeFolder === 'sent'" class="min-w-full text-base select-none">
                            ...
                            <tbody>
                                <tr v-for="(msg, idx) in filteredPaginatedMessages" :key="msg.id" class="hover:bg-blue-50 dark:hover:bg-blue-800 cursor-pointer transition border-b border-gray-100 dark:border-gray-800" @click.stop="selectMessage(msg)">
                                    <td><input type="checkbox" :value="msg.id" v-model="selectedMessages"></td>
                                    <td class="pl-8 pr-4 py-3 text-black dark:text-white">{{ msg.sender }}</td>
                                    <td class="pl-8 pr-4 py-3 text-black dark:text-white">{{ msg.subject }}</td>
                                    <td class="pl-8 pr-4 py-3 truncate max-w-xs text-black dark:text-white">{{ stripHtml(msg.message) }}</td>
                                    <td class="pl-8 pr-4 py-3 whitespace-nowrap text-black dark:text-white">{{ msg.time }}</td>
                        ...
                        <table v-if="activeFolder === 'important'" class="min-w-full text-base select-none">
                            ...
                            <tbody>
                                <tr v-for="(msg, idx) in filteredImportantMessages" :key="msg.id" class="hover:bg-blue-50 dark:hover:bg-blue-800 cursor-pointer transition border-b border-gray-100 dark:border-gray-800" @click.stop="selectMessage(msg)">
                                    <td><input type="checkbox" :value="msg.id" v-model="selectedMessages"></td>
